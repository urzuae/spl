...
global $db, $uid, $how_many, $from, $orderby, $del, $rsort,$_site_title;
$_site_title = "Monitoreo vendedores-SFG0011";
$kpi_contactos = array();
...
$tabla_campanas .= "
                   ...
                   <thead><tr>"
                    ."<th class='tdcenter'>Nombre del vendedor</th>".$columnas_tabla.""
                    ."<th class='tdcenter'>Total</th>"
                    ."<th class='tdcenter'>Retrasos<br>acumulados</th>"
                    ."<th class='tdcenter'>Número<br>Retrasos </th>"
                    ."<th class='tdcenter'>Número<br>Ventas</th>"
                    ."<th class='tdcenter'>Prospectos<br> capturados al día</th>"
                    ."</tr>"
                    ."</thead>\n";
...
foreach ($array_para_ordenar AS $c_uid=>$x)
{
  $uname = $users[$c_uid];
	...
	$tabla_campanas .= "<td class='tdleft'><a href=\"index.php?_module=$_module&_op=monitoreo_vendedor&uid=$c_uid\">$uname</a></td>";
	$total_contactos = 0;
	foreach ($campanas AS $campana_id=>$name)
	{
		$tabla_campanas .= "<td class='tdright'>".($contactos[$c_uid][$campana_id])."</td>";
	...
	$tabla_campanas .= "<td class='tdright'>$total_contactos</td>";
	$tabla_campanas .= "<td class='tdright' id=\"kpi_retrasos_$c_uid\">$retrasos_acumulados hr</td>";
	$tabla_campanas .= "<td class='tdright' id=\"kpi_retrasos_cuantos_$c_uid\">".($retrasos + 0)."</td>";
	$tabla_campanas .= "<td class='tdright' id=\"kpi_ventas_$c_uid\">".($ventas + 0)."</td>";
	$tabla_campanas .= "<td class='tdright'id=\"kpi_contactos_$c_uid\">$kpi_contactos[$c_uid]</td>";
	$tabla_campanas .= "</tr>";
	$ventas_total += $ventas;
...
if ($total_users)
{
	...
	$tabla_campanas .= "<tr class=\"row".(($c++%2)+1)."\"><td class='tdright'>Total</td>";
	foreach ($campanas AS $campana_id=>$name)
	{
		$tabla_campanas .= "<td class='tdright'>".$total_campanas[$campana_id]."</td>";
		$total_total += $total_campanas[$campana_id];
	}
	
	$tabla_campanas .= "<td class='tdright'>$total_total</td>";
	$tabla_campanas .= "<td class='tdright' id=\"total_kpi_retrasos\">$retrasos_acumulados_total hr</td>";
	$tabla_campanas .= "<td class='tdright' id=\"total_retrasos_cuantos\">$retrasos_total</td>";
	$tabla_campanas .= "<td class='tdright' id=\"total_ventas\">$ventas_total</td>";
	$tabla_campanas .= "<td class='tdright' id=\"total_kpi_contactos\">$kpi_contactos_total</td>";
	$tabla_campanas .= "</tr>\n";
}
