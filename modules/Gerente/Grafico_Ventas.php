...
class Grafico_Ventas
{
    ...
    function Regresa_Imagen()
    {
        ...
        $xml="<chart palette='1' showBorder='1' caption='Comparación de ventas: objetivos contra real".$nm_subprograma."' showValues='0' decimals='0' formatNumberScale='0' yAxisMinValue='0' yAxisMaxValue='100' exportEnabled='1' exportAtClient='1' exportHandler='fcExporter".$contador_div."c' exportType='PNG=Exportar como imagen'><categories>";
        ...
        for ($j = 1; $j <= 12; $j++)
        {
            ...
            $xml.="<set value='".$total_mes. "' showLabel='1' link='".$url."' toolText='Objetivo de venta del mes:  ".$this->array_meses[str_pad($j,2,'0',STR_PAD_LEFT)]."\nObjetivo:   ".$this->array_metas[$j]."\nVenta:  ".$this->array_ventas[$j]."\nPorcentaje: ".$prom."'/>";
	...
	$xml.="</dataset>";
	$xml.="</chart>";
        $buf.="<table style='border:0px;'><tr><td>";
        #$buf.=renderChartHTML("includes/fusion/StackedColumn3D.swf", "", $xml, "Proyección de Metas y Ventas por mes", 750, 350, false, false);
        $buf.=renderChartHTML("includes/fusion/MSColumnLine3D.swf", "", $xml, "Objetivos y Ventas por mes", 750, 350, false, false);
    ...
    function Regresa_Tabla()
    {
        $buf.="
                <table width='100%' align='center' border='0' class='tablesorter'>
                 <thead><tr heigth='30'><th>&nbsp;</th>";
        foreach($this->array_meses as $mes_id => $nm_mes)
        {
            $buf.="<th align='center'>".$nm_mes."</th>";
        }
        $buf.="<th>Anual</th></tr></thead><tbody><tr class='row2'><td>Objetivos de<br>ventas</td>";
        $total_anual=0;
        ...
        $buf.="</tr><tr class='row1'><td>Ventas<br>reales</td>";
        $ventas_anual=0;
        ...
        $buf.="</tr><tr class='row2'><td>Porcentaje</td>";
        $promedio_anual=0;
        foreach($this->array_metas as $mes_id => $valor)
        {
            $prom_mensual=0;
    ...
    function Consulta_informacion_Ventas()
    {
        $sql="SELECT year(a.timestamp) AS ano, month(a.timestamp) AS mes,sum(a.precio) AS total
              ...
              GROUP BY substr(a.timestamp,1,7)
              ORDER BY substr(a.timestamp,1,7)";
        ...
        //Checar en contactos_finalizados
        $sql="SELECT year(a.timestamp) AS ano, month(a.timestamp) AS mes,sum(a.precio) AS total
              FROM crm_prospectos_ventas as a left join crm_contactos_finalizados as b on a.contacto_id=b.contacto_id
              WHERE a.eliminar = 0 AND year(a.timestamp) ='".$this->ano_id."' AND b.gid='".$this->gid."' ".$this->filtro."
              GROUP BY substr(a.timestamp,1,7)
              ORDER BY substr(a.timestamp,1,7)";
        $res=$this->db->sql_query($sql) or die ("Error en la consulta:  ".$sql);
        if($this->db->sql_numrows($res) > 0)
        {
            ...
            {
                $cantidad_ventas=$cantidad_ventas+0;
                $this->array_ventas[$mes]=$this->array_ventas[$mes] + $cantidad_ventas;
