...
<div id=\"bloque_$campana_id\" style=\"display:block;\">";
    ...
    $tabla_campanas .= "<table id=\"tabla_contactosven$contador_tablas\" class=\"tablesorter\" >"
                    ."<thead>"
                    ."<tr>"
                    ."<th class='tdcenter' style=\"width:180px;\">Campaña</th>"
                    ."<th class='tdcenter' style=\"width:330px;\">Nombre</th>"
                    ."<th class='tdcenter' style=\"width:170px;\">Producto</th>"
                    ."<th class='tdcenter' style=\"width:140px;\">Fecha Registro</th>"
                    ."<th class='tdcenter' style=\"width:170px;\">Último contacto</th>"
                    ."<th class='tdcenter' style=\"width:170px;\">Compromiso</th>"
                    ."<th class='tdcenter' style=\"width:150px;\">Retraso (hrs)</th>"
                    ."<th class='tdcenter' style=\"width:32px;\">Sel.</th></tr>"
                    ."</thead>"
                    ."<tbody>";
	...
	foreach ($ordered_contacto_ids AS $contacto_id)
	{
      ...
      $tabla_campanas .= "<tr class=\"row".(($c++%2)+1)."\">"
                        ...
                        ."<td class='tdleft'><a href=\"index.php?_module=Campanas&_op=llamada_ro&llamada_id={$llamada_ids[$contacto_id]}&contacto_id=$contacto_id&campana_id={$campana_ids[$contacto_id]}\" target=\"llamada\">
                        $nombre $apellido_paterno $apellido_materno&nbsp;&nbsp;<span style='background-color:$semaforo'>&nbsp;&nbsp;&nbsp;</span></a></td>"
                        ."<td class='tdleft'>".Regresa_producto($db,$contacto_id)."</td>"
                        ."<td class='tdleft'>".substr($espera,0,10)."</td>"
                         ."<td class='tdleft'>$ultimo_contacto</td>"
                         ."<td class='tdleft'>$fecha_cita</td>"
                         ."<td class='tdleft'>$retraso</td>"
                         ."<td class='tdcenter'><input type=\"checkbox\" name=\"chbx_$contacto_id\" style=\"height:12;width:16;\"></td>"
                         ."</tr>";
	}
	$tabla_campanas .= "</tbody><thead><tr class=\"row".(($c++%2)+1)."\"><td class=\"tdright\">Total</td><td colspan='7' class='tdleft'>$counter</td></tr></thead>";
...
if ($counter_total)
{
    $tabla_campanas .=
        ...
        <tbody> <tr><th class='tdcenter'>Total $counter_total</th></tr></tbody>
        </table>";
    $tabla_campanas .= "
        ...
        <tr class=\"row".(++$row_class%2+1)."\">
            <td colspan='7' class='tdcenter'>
            <input type=\"submit\" name=\"seleccionar\" value=\"Reasignar\">
            </td>
        </tr>
        </table>";
}
...
function Regresa_producto($db,$contacto_id)
{
    $prods='';
    $sql = "SELECT modelo_id,modelo FROM crm_prospectos_unidades WHERE contacto_id=".$contacto_id." order by modelo;";
    $result2 = $db->sql_query($sql) or die("Error" .$sql);
