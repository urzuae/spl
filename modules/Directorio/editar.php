...
$fecha_de_nacimiento = "$ano-$mes-$dia";
    ...
    {    
        ...
        $result = $db->sql_query($sql) or die("Error al consultar datos del contacto");
        list($modelo, $version, $ano_auto, $tipo_pint, $color_ext, $color_int, $idVehiculo, $idVersion, $idTransmision) = $db->sql_fetchrow($result);
        $modelo_current = $modelo;
    ...
    $_subtitle = "$nombre $apellido_paterno $apellido_materno - &Uacute;ltima modificaci&oacute;n: $timestamp2";
...
{
  $selected_prod = $name_prod == $modelo_current ? "selected=\"selected\"" : "";
  $select_product .= "<option value=$id_prod $selected_prod>$name_prod</option>";
}
